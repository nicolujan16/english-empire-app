...
interface CompraEspecialModalProps {
	compra: CompraEspecial | null;
	isOpen: boolean;
	onClose: () => void;
}
...
export default function CompraEspecialModal({
	compra,
	isOpen,
	onClose,
}: CompraEspecialModalProps) {
	const printRef = useRef<HTMLDivElement>(null);

	if (!compra) return null;

	const numeroComprobante = `CMP-${compra.id.slice(-8).toUpperCase()}`;
	...
	const handlePrint = () => {
		...
		ventana.document.write(`
			<html>
				<head>
					<title>Comprobante de Compra — ${compra.compradorDNI}</title>
					<style>
					...
					<div class="comprobante">
						...
						<div class="header">
							<div>
								...
								<p class="subtitulo">Comprobante oficial de compra</p>
							</div>
						...
						<div class="sello">
							<div>
								<p class="sello-title">✓ Compra registrada correctamente</p>
								<p class="sello-sub">${fechaFormateada} — ${compra.metodoDePago}</p>
							</div>
						</div>
						...
						<div class="grid">
							<div>
								...
								<div class="row">
									...
									<span class="row-value">${compra.compradorDNI}</span>
								</div>
							</div>
							<div>
								...
								<div class="row">
									...
									<span class="row-value">${compra.metodoDePago}</span>
								</div>
							</div>
						</div>
						...
						<div class="footer">
							<p>Este documento es un comprobante válido de compra emitido por English Empire Institute.</p>
					...
					<div ref={printRef}>
						...
						<div className="flex items-center gap-2.5 bg-green-50 border border-green-200 rounded-lg px-4 py-3 mb-6">
							...
							<div>
								...
								<p className="text-xs text-green-600">
									{fechaFormateada} — {compra.metodoDePago}
								</p>
							</div>
						</div>
						...
						<div className="border border-gray-200 rounded-lg overflow-hidden mb-6">
							<table className="w-full text-sm">
								...
								<tfoot className="bg-[#252d62]">
									<tr>
										<td className="px-4 py-3 text-xs font-bold text-white text-right">
											TOTAL ABONADO
										</td>
						...
						<div className="border-t border-gray-100 pt-4 text-center">
							<p className="text-[10px] text-gray-400 leading-relaxed">
								Este documento es un comprobante válido de compra emitido
