...
interface GestionarIngresosModalProps {
	isOpen: boolean;
	onClose: () => void;
	onSuccess: () => void;
}

export default function GestionarIngresosModal({
	isOpen,
	onClose,
	onSuccess,
}: GestionarIngresosModalProps) {
	...
	const fetchCategorias = async () => {
		...
		setError(null);
		try {
			const docRef = doc(db, "Configuraciones", "CategoriasIngreso");
	...
	const handleSaveToDB = async () => {
		...
		setError(null);
		try {
			await setDoc(doc(db, "Configuraciones", "CategoriasIngreso"), {
				nombres: categorias
			});
			setExito(true);
			setTimeout(() => {
			...
			<DialogContent className="sm:max-w-[500px] p-0 overflow-hidden">
					...
					<div className="flex items-center gap-3">
						...
						<div>
							...
							<p className="text-xs text-gray-500 mt-0.5">
								Administrá, ordená y agregá categorías para los ingresos especiales
							</p>
						</div>
					</div>
				</DialogHeader>
				...
				<div className="px-6 py-5 flex flex-col gap-4 max-h-[70vh] overflow-y-auto">
												...
												<div className="flex items-center gap-3 w-full pr-16">
													...
													{editingIndex === index ? (
														<input
															...
															autoFocus
															className="flex-1 text-sm font-semibold text-gray-900 border-b border-[#252d62] bg-transparent outline-none focus:ring-0 px-0 py-0"
														/>
													) : (
												...
												{editingIndex !== index && (
													<div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center opacity-0 group-hover:opacity-100 transition-opacity bg-white pl-2">
														<button
															...
															className="p-1.5 text-gray-400 hover:text-[#252d62] hover:bg-blue-50 rounded-md transition-colors"
															title="Editar"
														>
					...
					<p className="text-[10px] text-gray-400 text-center">
						Arrastrá las opciones con el ícono de la izquierda para cambiar el orden en que aparecerán al registrar un ingreso.
					</p>
				</div>
