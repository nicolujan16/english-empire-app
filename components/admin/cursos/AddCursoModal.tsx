...
// Nueva interfaz para manejar los horarios en el formulario
...
export default function AddCursoModal({
	...
	const [horariosList, setHorariosList] = useState<HorarioItem[]>([
		{ dia: "Lunes", horaInicio: "18:00", horaFin: "19:30" }, // Fila por defecto
	]);
	...
	useEffect(() => {
		if (courseToEdit && isOpen) {
			// PARSEAR HORARIOS DE BD AL FORMATO DEL FORMULARIO
			...
			if (!isTBD && Object.keys(courseToEdit.horarios).length > 0) {
				Object.entries(courseToEdit.horarios).forEach(([dia, horas]) => {
					// Asumimos que la BD guarda "18:30 - 20:30"
				...
				// Si está a definir o vacío, dejamos uno por defecto oculto
			...
			setFormData({
				...
				cuota:
					courseToEdit.cuota !== undefined ? courseToEdit.cuota.toString() : "",
				inscripcion:
					courseToEdit.inscripcion !== undefined
						? courseToEdit.inscripcion.toString()
						: "",
				edadMin: courseToEdit.edades?.[0]?.toString() || "",
				edadMax: courseToEdit.edades?.[1]?.toString() || "",
				mesInicio: courseToEdit.inicio || "Marzo",
				mesFin: courseToEdit.fin || "Diciembre",
	...
	const handleSubmit = async (e: SyntheticEvent) => {
		...
		try {
			...
			// Procesar horarios dinámicos al formato Record<string, string>
			...
			if (isScheduleTBD) {
				...
				horariosList.forEach((item) => {
					if (item.dia) {
						// Si tiene fin, formatea "18:00 - 19:30". Si no, solo "18:00"
						const horaTexto = item.horaFin
							? `${item.horaInicio} - ${item.horaFin}`
							: item.horaInicio;

						// Si se seleccionó el mismo día dos veces, los junta (ej: "18:00, 20:00")
			...
			const courseData: any = {
				...
				horarios: horariosObj, // Guardamos el objeto formateado
				inicio: formData.mesInicio,
				fin: formData.mesFin,
