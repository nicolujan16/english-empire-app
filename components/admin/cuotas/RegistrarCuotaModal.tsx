...
export default function RegistrarCuotaModal({
	...
	const handleRegistrarPago = async () => {
		...
		try {
			...
			// ENVÍO DE CORREO DE COMPROBANTE ─────────────────────────────────────
			let emailDestino = "";
			try {
				const esMenor = cuotaACobrar.alumnoTipo.toLowerCase() === "menor";

				if (esMenor) {
					const hijoSnap = await getDoc(
						doc(db, "Hijos", cuotaACobrar.alumnoId),
					);
					if (hijoSnap.exists()) {
					...
					const userSnap = await getDoc(
						doc(db, "Users", cuotaACobrar.alumnoId),
					);
					if (userSnap.exists()) {
				...
				console.error(
					"Error buscando el email del alumno/tutor para la cuota:",
					e,
				);
			}

			// Si encontramos un email, disparamos a la API de correos
			if (emailDestino !== "") {
				try {
					await fetch("/api/correos/cuota", {
						method: "POST",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({
							emailDestino: emailDestino,
							nombreAlumno: cuotaACobrar.alumnoNombre,
							cursoNombre: cuotaACobrar.cursoNombre,
							mes: cuotaACobrar.mes,
							anio: cuotaACobrar.anio,
							montoAbonado: montoMostrar,
							metodoPago: paymentMethod,
							// Usamos el ID de la cuota como número de transacción interno
							nroComprobante: `TXN-${cuotaACobrar.id.slice(-8).toUpperCase()}`,
						}),
					});
					console.log(
						"✉️ Comprobante de cuota enviado por correo a:",
						emailDestino,
					);
				} catch (emailError) {
					console.error(
						"❌ Error enviando el comprobante de cuota por mail:",
						emailError,
					);
				}
			}
	...
	const mejorDescuento = cuotaACobrar
		? obtenerMejorDescuento(cuotaACobrar.descuentos)
		: null;
	const tieneDescuentos = !!mejorDescuento;
	const precioBaseOriginal = cuotaACobrar
		? calcularPrecioBase(cuotaACobrar)
		: 0;
										...
										{isSplitPayment && (
											...
											>
												...
												{saldoRestante > 0 && (
													...
													>
														<Plus className="w-3.5 h-3.5 mr-1.5" /> Agregar otro
														pago por ${saldoRestante.toLocaleString("es-AR")}
													</Button>
												)}

												{saldoRestante < 0 && (
													<p className="text-[10px] text-red-500 font-medium text-center">
														Los montos superan el total a cobrar. Ajuste los
														valores.
													</p>
												)}
											</motion.div>
